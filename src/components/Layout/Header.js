...
const Header = ({ props, dispatch }) => {
  ...
  const toggleSideBar = () => {
    ...
    setTimeout(
      () =>
        isOpen
          ? document.body.classList.remove("disbaled-scroll")
          : document.body.classList.add("disbaled-scroll"),
      500
    );
  };
  ...
  return (
    <>
      ...
      <header className="flex justify-center items-center justify-between m-4">
        <span className="float-left menu_btn" onClick={toggleSideBar}>
          <span></span>
          <span></span>
          <span></span>
        </span>
        ...
        {!isMobile && (
          <Link href="/contact-us">
            <a className="float-right btn">Grow your business</a>
          </Link>
        )}
      </header>
